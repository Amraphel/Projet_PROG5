...
Tab_Sec* renumerotation_table_section(Elf32Hdr * header, Tab_Sec* sect, int * tab_renum){
	...
	Tab_Sec * sect2 = malloc(sizeof(Tab_Sec) * (header->e_shnum-cpt_rel));
	int num=0;
	ELF32_Word type2 ;
	for (int i = 0; i < header->e_shnum; i++){
	...
	// header->e_shnum=header->e_shnum-cpt_rel;
	return sect2;
}

void renumerotation_table_symbole(FILE* file,Elf32_Sym* tab_sym ,Elf32Hdr header,Tab_Sec* tab_sec, int* tab_renum){
...
void trier_table_symbole(Elf32_Sym* tab_sym,Elf32Hdr header,Tab_Sec* tab_sec,FILE* file){
    int taille = get_taille_table_symbole(file, tab_sec, header);
    int indice=1;
    for(int i=1; i<taille; i++){
